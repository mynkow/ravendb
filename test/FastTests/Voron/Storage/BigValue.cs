...
namespace FastTests.Voron.Storage
{
    public class BigValues : StorageTest
    {
        ...
        public void CanReuseLargeSpace(int restartCount)
        {
            ...
            {
                var tree = tx.CreateTree("foo");
                tree.Delete(new SliceArray(BitConverter.GetBytes(1203)));
                tx.Commit();
            }
        ...
        public void CanStoreInOneTransactionManySmallValues()
        {
            ...
            using (var tx = Env.WriteTransaction())
            {
                ...
                for (int i = 0; i < 1500; i++)
                {
                    ...
                    random.NextBytes(buffer);
                    buffers.Add(buffer);
                    tree.Add<SliceArray>(new SliceArray(BitConverter.GetBytes(i)), new MemoryStream(buffer));
                }
                tx.Commit();
            }
            ...
            using (var tx = Env.ReadTransaction())
            {
                ...
                for (int i = 0; i < 1500; i++)
                {
                    var readResult = tree.Read(new SliceArray(BitConverter.GetBytes(i)));
                    Assert.NotNull(readResult);
